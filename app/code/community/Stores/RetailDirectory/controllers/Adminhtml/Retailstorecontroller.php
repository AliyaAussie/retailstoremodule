...
class Stores_RetailDirectory_Adminhtml_Retailstorecontroller extends Mage_Adminhtml_Controller_Action{
    ...
    public function indexAction(){
        ...
        $retailstoreBlock = $this->getLayout()->createBlock('stores_retaildirectory_adminhtml/retailstore');
    ...
    {
        $retailstore = Mage::getModel('smashingmagazine_retaildirectory/brand');
        if ($retailstoreId = $this->getRequest()->getParam('id', false)) {
            $retailstore->load($retailstoreId);
        }
        ...
        try {
            ...
            $this->_getSession()->addSuccess(
                $this->__('The retail store has been deleted.')
            );
        } catch (Exception $e) {
            Mage::logException($e);
            $this->_getSession()->addError($e->getMessage());
